 ...
 * Run with: npx vitest run (or node --experimental-vm-modules if using vitest)
 */
...
function testStreak() {
  ...
  console.log(result1 === 0 ? "  ✅ No check-ins → 0" : "  ❌ Failed");

  // Test 2: Single check-in today
  const today = new Date().toISOString().split("T")[0];
  const result2 = calculateStreak([today]);
  console.assert(result2 === 1, `Expected 1, got ${result2}`);
  console.log(result2 === 1 ? "  ✅ Today only → 1" : "  ❌ Failed");

  // Test 3: Streak of 3 consecutive days
  const d = new Date();
  const dates = [];
  for (let i = 0; i < 3; i++) {
    const d2 = new Date(d);
  ...
  console.log(result3 === 3 ? "  ✅ 3 consecutive days → 3" : "  ❌ Failed");

  // Test 4: Streak broken (gap)
  const d4 = new Date();
  const dates4 = [
    new Date(d4).toISOString().split("T")[0],
    new Date(d4.setDate(d4.getDate() - 2)).toISOString().split("T")[0],
  ];
  const result4 = calculateStreak(dates4);
  console.assert(result4 === 1, `Expected 1, got ${result4}`);
  console.log(result4 === 1 ? "  ✅ Broken streak → 1" : "  ❌ Failed");
  ...
  console.log(result5 === 1 ? "  ✅ Duplicate dates → 1" : "  ❌ Failed");
}
...
function calculateProgress(submissions: number, total: number): number {
  if (total <= 0) return 0;
...
function testProgress() {
  ...
  console.log(
    calculateProgress(12, 24) === 50 ? "  ✅ 12/24 = 50%" : "  ❌ Failed",
  );

  console.assert(calculateProgress(0, 20) === 0, "0/20 should be 0%");
  console.log(
    calculateProgress(0, 20) === 0 ? "  ✅ 0/20 = 0%" : "  ❌ Failed",
  );

  console.assert(calculateProgress(20, 0) === 0, "20/0 should be 0%");
  console.log(
    calculateProgress(20, 0) === 0 ? "  ✅ 20/0 = 0%" : "  ❌ Failed",
  );

  console.assert(calculateProgress(1, 3) === 33, "1/3 should be 33%");
  console.log(
    calculateProgress(1, 3) === 33 ? "  ✅ 1/3 = 33%" : "  ❌ Failed",
  );
}

// ─── Rubric Total ────────────────────────────────────────────────────────────
function sumRubricPoints(items: { maxPoints: number }[]): number {
...
// ─── Smart Goal Progress ─────────────────────────────────────────────────────
function goalProgress(milestones: { completed: boolean }[]): number {
...
function testGoalProgress() {
  ...
  console.log(
    goalProgress([]) === 0 ? "  ✅ Empty milestones = 0%" : "  ❌ Failed",
  );
  ...
  console.assert(
    goalProgress([{ completed: true }, { completed: true }]) === 100,
    "2/2 → 100%",
  );
  console.log(
    goalProgress([{ completed: true }, { completed: true }]) === 100
...
function testInitials() {
  ...
  console.log(
    getInitials("Dr. Smith") === "DS" ? "  ✅ Dr. Smith → DS" : "  ❌ Failed",
  );
